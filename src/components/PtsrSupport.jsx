...
const PtsrSupport = () => {
  ...
  return (
    <div>
      <div className="m-2 pr-10 grid grid-cols-1  sm:grid-cols-2 lg:grid-cols-3 gap-4 items-stretch">
        {posts.map((post) => (
          <Link key={post.id} to={`/ptsr/${post.id}`} className="h-full">
            <div className="flex flex-col justify-center items-center bg-white/10 backdrop-blur-md border-white/30 rounded-3xl shadow-2xl p-8 m-5 max-w-md w-full h-full hover:bg-white/30 transition duration-300 ease-in-out">
