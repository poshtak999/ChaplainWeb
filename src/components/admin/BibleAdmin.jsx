...
const BibleAdmin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingPost) {
        await api.updateBiblePost(editingPost.id, formData);
        setSuccess('Пост успішно оновлено!');
      } else {
        await api.createBiblePost(formData);
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="bg-white/10 backdrop-blur-md border-white/30 rounded-3xl shadow-2xl p-6 mb-8">
          <h2 className="text-3xl text-amber-50 font-bold mb-4">
            {editingPost ? '✏️ Редагувати пост' : '➕ Додати новий пост'}
          </h2>
        ...
        <div className="space-y-4">
          ...
          {posts.map((post) => (
            ...
            >
              ...
              <p className="text-amber-50 mb-4">{post.description}</p>
              <div className="flex gap-4">
                <button
